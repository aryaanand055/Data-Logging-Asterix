...
def main() -> None:
    parser = argparse.ArgumentParser(description='Hall-effect speed JSONL -> SQLite uploader')
    parser.add_argument('--db-path', default=str(SHARED_DB_PATH), help='SQLite database file path')
    parser.add_argument('--sensor-name', default='hall_effect_speed', help='Sensor table name prefix')
    parser.add_argument('--source-id', default='hall_effect_speed_01', help='Source ID written with each sample')
    ...
    if not args.quiet:
        print(f'Logging hall-effect speed data to SQLite: {args.db_path}', file=sys.stderr)
    ...
    while RUNNING:
        ...
        if not args.quiet:
            print(f'logged hall-effect speed sample {payload.get("sequence", "?")}', file=sys.stderr)

    if not args.quiet:
        print('Hall-effect speed uploader stopped.', file=sys.stderr)
